...
class ProjectStatusViewModel(val repo: ProjectStatusRepo,val context: Context):ViewModel() {

    var liveData:MutableLiveData<Response<ProjectStatusResponse>>
    init {
        liveData = MutableLiveData()
        getProjectStatus()
    }

    private fun getProjectStatus() {
        viewModelScope.launch {
            liveData.postValue(Response.Loading(loadingMessage = context.resources.getString(R.string.LOADING_PLEASE_WAIT)))
            if (NetworkUtils.isNetworkAvailable(context)) {
                try {
                    val response = repo.callUserProjectDetails()
