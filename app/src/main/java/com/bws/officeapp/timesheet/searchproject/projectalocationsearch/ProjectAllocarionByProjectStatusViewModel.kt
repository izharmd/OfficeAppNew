...
class ProjectAllocarionByProjectStatusViewModel(val repo: ProjectAllocationRepoByProjectStatus,val context: Context):ViewModel(){
    var liveData: MutableLiveData<Response<ProjectAllocationResponse>>

    init {
        liveData = MutableLiveData()
        getProjectAllocationByProjectStatus()
    }

    private fun getProjectAllocationByProjectStatus() {
        viewModelScope.launch {
            liveData.postValue(Response.Loading(loadingMessage = context.resources.getString(R.string.LOADING_PLEASE_WAIT)))
            if (NetworkUtils.isNetworkAvailable(context)) {
                try {
                    val response = repo.callProjectAllocationByProjectStatus()
