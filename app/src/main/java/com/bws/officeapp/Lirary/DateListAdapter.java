...
public class DateListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    class DateViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setDates(final DateModel model) {
            ...
            Date date_current_month_date = main_calendar.getTime();
                ...
                public void onClick(View v) {
                    ...
                    for (int i = 0; i < AppConstants.eventList.size(); i++) {
                        if (AppConstants.eventList.get(i).getStrDate().equals(AppConstants.sdfDate.format(model.getDates()))) {
                            ...
                            // String st = AppConstants.eventList.get(i).getStrName();
                            ...
                            myEventModel.setEventStartDate(eventModel.getStrDate());
                            myEventModel.setEventEndDate(eventModel.getStrEndDate());
                          //  myEventModel.setEventName(eventModel.getStrName());
                            myEventModel.setProjectStatus(eventModel.getProjectStatus());
                    ...
                    if (m != 0) {
                        Common.dateCalendarHeader = sdfMonthYear.format(main_calendar.getTime());
                        String dt = main_calendar.getTime().toString();
                        System.out.println("DDDDDD===" + dt);
                        //  tv_month_date.setText(String.valueOf(model.getDates().getDate()))
                        String tv_month_date = String.valueOf(model.getDates().getDate());
                        System.out.println("DDDDDD===" + tv_month_date + "-" + Common.dateCalendarHeader);
                        eventDialog();
                        /*int currentYear = main_calendar.get(Calendar.YEAR);
                        int currentMonth = main_calendar.get(Calendar.MONTH);
                        ...
                        Toast.makeText(context,"Today's Date: " + mDate, Toast.LENGTH_SHORT).show();
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        ((DateViewHolder) holder).tv_month_date.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int color = ((DateViewHolder) holder).tv_month_date.getCurrentTextColor();
                String hexColor = String.format("#%06X", (0xFFFFFF & color));
                System.out.println("Today's Date: " + hexColor);
                if (hexColor.equalsIgnoreCase("#000000") ||
                        hexColor.equalsIgnoreCase("#0000FF")) {
                    String currentDay = ((DateViewHolder) holder).tv_month_date.getText().toString();
                    int currentYear = main_calendar.get(Calendar.YEAR);
        ...
        ((DateViewHolder) holder).tv_week_date.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int color = ((DateViewHolder) holder).tv_month_date.getCurrentTextColor();
                String hexColor = String.format("#%06X", (0xFFFFFF & color));
                ...
                if (hexColor.equalsIgnoreCase("#000000") ||
                        hexColor.equalsIgnoreCase("#0000FF")) {
                    String currentDay = ((DateViewHolder) holder).tv_week_date.getText().toString();
                    int currentYear = main_calendar.get(Calendar.YEAR);
                    int currentMonth = main_calendar.get(Calendar.MONTH) + 1;
                    System.out.println("Today's Date: " + currentYear + currentMonth + currentDay);
                }
